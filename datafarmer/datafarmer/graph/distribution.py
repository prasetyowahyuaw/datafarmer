...
def generate_box_plot(df: pd.DataFrame, labels_name: str= None, values_name: str= None) -> plt.Figure:
    ...
    for i, box in enumerate(box_props["boxes"]):
        ...
        add_annotation(median, f'Median: {median:.2f}', 'darkred', offset=0)
        add_annotation(lower_quartile, f'Q1: {lower_quartile:.2f}', 'darkblue', offset=0)
        add_annotation(upper_quartile, f'Q3: {upper_quartile:.2f}', 'darkblue', offset=0)
        add_annotation(lower_whisker, f'Lower : {lower_whisker:.2f}', 'grey', offset=0)
        add_annotation(upper_whisker, f'Upper : {upper_whisker:.2f}', 'grey', offset=0)
